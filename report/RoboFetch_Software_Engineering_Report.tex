...
{Component diagram in UML~2 notation. Each component carries \emph{ports} on its boundary, and
every dependency is drawn as an \emph{assembly connector}: a provided interface (the ball) meeting
a required one (the socket). The Web Tier is the only component opened up, so that
