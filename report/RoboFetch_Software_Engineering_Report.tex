...
The project follows a single, named process model: \textbf{plan-driven incremental delivery}. The
specification and the architecture are settled once, before construction begins, as in a waterfall
process; the construction itself is then delivered as a series of increments, each of which is a
working, verified subset of the system rather than a stage of a partially built one. Sommerville
describes this as incremental delivery, and it is sometimes called staged delivery. It is one
model, not two run side by side, and the report is organised to match it: a section for each
up-front phase, and a section for each increment of construction.

\subsection*{Why the specification is settled up front}
Three properties of this problem make the plan-driven half appropriate.

First, the requirements were knowable before implementation started. The requirements of a delivery
robot are determined largely by physics and by the warehouse, and both are known in advance; they
are not discovered through user feedback. Second, the client specifies once rather than
collaborating continuously, so there is no channel through which a changing scope would arrive.
Third, the architecture is load-bearing and expensive to revise: the boundary between the web tier,
the robotic middleware and the simulator determines what can be tested and how, and settling it
late would have invalidated work already done.

\subsection*{Why construction is nevertheless incremental}
A pure waterfall would have required the whole system to be built before any of it could be
demonstrated, and in this project that is not merely inconvenient but impossible to verify. The
...
layer depends on the one beneath it behaving correctly in the world rather than on paper.

Construction therefore proceeded as increments, each ending in something that ran and was verified
against the simulator before the next began. Navigation could not be written until a robot could be
driven; manipulation could not be written until the robot could reach a shelf; the decision layer
could not be written until an order could be executed. The increments follow that dependency order,
which is a property of the problem rather than a schedule anyone chose.

This is also what made two later decisions safe. An increment that has been delivered and verified
can be judged on its behaviour rather than on its design, and two of them were subsequently
\emph{withdrawn} on that evidence: the nearest-neighbour scheduler, because orders should be served
in the sequence customers placed them, and the retry state machine, because its complexity was not
repaid by what it did. Removing a delivered increment is a decision a plan-driven process has no
natural place for, and it is one of the clearest arguments that the incremental half of this model
was doing real work.

\subsection*{Why not an agile process}
The distinction that matters is between \emph{incremental delivery} and \emph{incremental
specification}. This project delivered incrementally but specified once. An agile process assumes
the requirements are discovered through repeated contact with users, and answers that with
artifacts built for negotiation: a product backlog that is reordered between iterations, user
stories written and re-estimated as understanding changes, and a velocity measure used to forecast.
None of those has anything to act on here, because there is no channel through which the
requirements change. Adopting them would have produced ceremony without information.
...
The table below gives the phases and what each produced. The first three are carried out once and
completed before construction starts, which is what makes the process plan-driven. Implementation
and testing are \emph{not} single passes: they are repeated inside every increment, and are listed
here once because the deliverable is the same each time. The increments themselves are enumerated
in Section~\ref{sec:increments}.
...
Implementation \emph{(per increment)} & The nine software packages, the simulated world, and the
machine-learning pipeline, delivered across the nine increments of
Section~\ref{sec:increments}. \\
Testing \emph{(per increment)} & The automated test suites, and the acceptance suite that runs
against the live system. Each increment was verified against the simulator before the next began;
the measured results are reported in Chapter~\ref{ch:testing}. \\
...
Construction was delivered as nine increments. Each one ends in something that runs and that was
verified before the next began, and each is listed below with what it delivered and the evidence
that closed it. The evidence is deliberately behavioural rather than a statement that the code was
written: an increment was not considered closed because it compiled, but because the system did
something observable that it could not do before.

The ordering is a consequence of dependency, not of a schedule. Nothing above a layer can be
verified until the layer beneath it works in the world: navigation cannot be tested without a robot
that moves, manipulation cannot be tested without a robot that can reach a shelf, and the decision
layer cannot be tested without an order that executes.

\begin{longtable}{@{}p{0.05\textwidth}p{0.31\textwidth}p{0.28\textwidth}p{0.30\textwidth}@{}}
\toprule
\textbf{\#} & \textbf{Increment} & \textbf{Delivered} & \textbf{Evidence that closed it} \\
\midrule
\endhead
I1 & Simulated world and a drivable robot & The warehouse description, a differential-drive robot
with a laser sensor and a gripper link, and the bridge to the middleware & Sensor, odometry,
transform and velocity topics all carried; the robot drives under manual command \\
I2 & Map and autonomous navigation & A map generated from the same geometry that defines the world,
localization, and the navigation stack configured & Goals from one side of the room to the other
and back both succeed; localization error 0.05 to 0.14 m \\
I3 & Manipulation & The gripper node, attaching and detaching parcels, and the check that a grab
really took hold & Grab, release and re-attach all verified; out-of-reach grabs refused \\
I4 & One complete order & The task manager, chaining navigation and manipulation into a delivery &
A full navigate, grab, carry, release sequence completes \\
I5 & Order sequencing \emph{(later withdrawn)} & A nearest-neighbour scheduler serving orders by
proximity & Served in a different sequence from the one submitted, as designed. Withdrawn: fairness
to the customer outweighs distance travelled, and FR6 now requires arrival order \\
I6 & Failure handling \emph{(partly withdrawn)} & A retry state machine with backoff around the
grab & Three attempts, then the order fails and the queue continues. The state machine was later
withdrawn as complexity that did not repay itself; the bounded three-attempt behaviour it protected
survives as FR8 \\
I7 & Web tier and persistence & The database, the web interface and the bridge carrying orders into
the robotic system & An order submitted over the network reaches the database, then the robot, and
its status returns to the database \\
I8 & Operator interface & The pages through which orders are placed and the system is observed &
Every page renders server-side, and a delivery ordered from a browser completes end to end \\
I9 & Decision layer and release & The condition model, the admission workflow, the prediction
service, the test suites and the documentation & Orders are refused with reasons; the acceptance
suite passes against the live simulator \\
\bottomrule
\caption{The nine increments, what each delivered, and the behaviour that closed it.}
\label{tab:increments}
\end{longtable}

\subsection*{The turning point}
The ninth increment is the one that changes what the system is. Increments one to eight build a
robot that \emph{carries out} orders; the ninth gives it a model of its own physical state and a
workflow that can \emph{refuse} them. Everything the report calls a complex functionality lives in
that increment, which is why it is the largest and why it is described separately in
Chapter~\ref{ch:complex}.

\subsection*{Two increments were withdrawn, and that is the point}
I5 and I6 were built, verified and then removed. A plan-driven process with a single delivery at
the end has no natural moment at which a completed feature can be judged on its behaviour and
deleted; incremental delivery does, and this project used it twice.

The scheduler was withdrawn because seeing it work made the wrong behaviour obvious: orders served
by proximity mean a customer can be overtaken indefinitely because their goods are further away.
That is a requirements judgement which only became clear once the behaviour could be watched, and
it is now fixed in FR6. The retry state machine was withdrawn because its structure was more
elaborate than the behaviour it produced; the guarantee it existed to provide is stated directly by
FR8 and is met by a plain bounded loop.

Both removals reduced the amount of code in the system, and the project treats that as a result
rather than a loss. What survived is recorded in the requirements; what did not is recorded here,
because a report that lists only the features that worked describes a project that did not happen.
...
the user requirement it refines. They are divided into functional requirements, which describe
behaviour, and non-functional requirements, which constrain how that behaviour is delivered.
...
\midrule
\endhead
\input{generated/acceptance-table.tex}
\bottomrule
...
A third round of work followed, prompted by using the system rather than by rethinking it. Operating
it made three absences obvious: there was no way to sign in, so every visitor had the same reach;
there was no way to stop the robot once it had started; and there was no way to ask it to go home.
The same period added the reservation of work already accepted, without which the admission decision
was being taken against a robot that was standing still only on paper. None of these changed the
shape of the system, which is why they were added to the second version rather than prompting a
third: each is a page, a queue entry or a database query fitted to machinery that already existed.
