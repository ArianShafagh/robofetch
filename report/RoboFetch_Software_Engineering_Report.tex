...
The main contribution of the project is a delivery system that makes an informed decision before
accepting work. The contribution is not the machine-learning model itself, which is a small
classifier built with a standard library. The contribution is the decision workflow around it: the
system builds a feature vector from live robot telemetry, the product catalogue and a route it
computes itself, consults the model, and then applies its own safety policy which is allowed to
overrule the model's answer.

A second contribution is the physical verification discipline described above, and a third is the
condition model, which links payload weight, distance, energy, temperature and wear into a single
set of coupled equations that both the robot and the web tier share.

\section{Structure of this Report}
The report follows the phases of the development process described in
Chapter~\ref{ch:process}. Chapter~\ref{ch:categorization} classifies the functionality of the
system into the three categories required by the course. Chapters~\ref{ch:requirements}
and~\ref{ch:analysis} cover requirements and analysis. Chapter~\ref{ch:architecture} describes the
software architecture in detail, and Chapter~\ref{ch:quality} discusses the engineering principles
and quality standards that the implementation follows. Chapters~\ref{ch:environment}
to~\ref{ch:ml} describe the simulated environment, the implementation, the complex algorithms and
the machine-learning component. Chapter~\ref{ch:testing} presents the testing strategy and the
measured results. The remaining chapters cover the evolution of the system, its limitations and
the conclusions.
...
\section{Balance of the Categories}
Table~\ref{tab:balance} and Figure~\ref{fig:balance} summarise the classification.
...
\midrule
A - Data and repository & 9 & Well covered. A relational model with six related tables, full
create-read-update-delete access, and derived statistics. \\
B - Third-party services & 8 & Well covered and genuinely varied: a robotics stack, a physics
simulator, a web framework, a database engine and a machine-learning library. \\
C - Complex, implemented here & 3 & The smallest category, but the deepest. Each item is a
multi-step algorithm rather than a single function. \\
\bottomrule
\end{tabular}
\caption{Balance across the three required categories.}
\label{tab:balance}
\end{table}

\uploadedfigure{Figures/fig-category-balance.png}{0.72\textwidth}
{Distribution of functionality across the three required categories.}{fig:balance}

The third category contains fewer items than the other two. This is a deliberate result rather
than an oversight. An earlier version of the system contained two additional complex features, a
delivery scheduler that chose the geographically nearest pending order and a retry state machine
with exponential backoff. Both were removed. The scheduler was removed because customers expect to
be served in the order they placed their requests, not in the order that is most convenient for the
robot, so the queue is now processed first-in-first-out. The retry state machine was removed
because the complexity of the machine was not justified by the behaviour it produced; three
attempts followed by a return to the station achieves the same requirement in a form that can be
read at a glance. Removing a feature that does not earn its complexity is a legitimate engineering
decision, and the report states it openly rather than keeping unnecessary code in order to inflate
a count.
...
Figure~\ref{fig:dbclass} shows the schema as a UML class diagram, in which each table is a class
with the \code{table} stereotype and each column carries its SQL type. Figure~\ref{fig:er} shows
the same schema as an entity-relationship diagram, which is the more conventional view of a
database and makes the cardinalities easier to read.

\uploadedfigure{Figures/07-database-class.png}{0.98\textwidth}
{The database schema as a UML class diagram. Primary and foreign keys are marked, and the two
single-row tables that intentionally have no relationships are annotated.}{fig:dbclass}

\uploadedfigure{Figures/06-er-diagram.png}{0.95\textwidth}
{The same schema as an entity-relationship diagram.}{fig:er}
...
Three quantities are integrated forward at each control step. For a step of duration $\Delta t$ in
which the robot covers a distance $d$ carrying a payload of mass $m$:

The energy consumed is
\begin{equation}
E = d \cdot (e_{base} + e_{load} \cdot m) \cdot f_{temp}(T) \cdot f_{health}(H)
\end{equation}

where the two penalty factors express that a hot or worn robot does the same work less efficiently:
\begin{align}
f_{temp}(T) &= 1 + \alpha \cdot \max(0, T - T_{ref}) \\
f_{health}(H) &= 1 + \beta \cdot \max(0, 1 - H/100)
\end{align}

The battery falls by the corresponding fraction of its capacity, or rises if the robot is docked.
The temperature follows a heating term proportional to motor load and payload, and a cooling term
proportional to the difference from ambient:
\begin{equation}
\frac{dT}{dt} = k_{heat} \cdot L \cdot (1 + \gamma \cdot m) - k_{cool} \cdot (T - T_{amb})
\end{equation}

The condition degrades with energy throughput and with time spent above a warning temperature:
\begin{equation}
\frac{dH}{dt} = -\left( w_{E} \cdot \dot{E} + w_{T} \cdot \max(0, T - T_{warn}) \right)
\end{equation}

Because $f_{health}$ feeds back into $E$, the three quantities are genuinely coupled rather than
independent.
...
and the admission workflow reads that state and refuses new work while it holds. Figures
\ref{fig:battery}, \ref{fig:temperature}, \ref{fig:duty} and \ref{fig:wear} show the model running
on the real system over one session.
...
Training a model requires thousands of labelled examples. A single delivery in the simulator takes
about a minute, so collecting them from real runs was not practical. The training set is therefore
generated synthetically, but generated by simulating runs through \emph{the same equations the live
robot uses}: the generator imports the condition model rather than reimplementing it. The output
uses the same file format that the live robot writes, so real and generated runs are
...
NFR1 & Preview responds within two seconds & 368 ms \\
...
condition model forward and calling the prediction service over HTTP, was measured at 368 and 420
milliseconds in two separate runs, against a requirement of two seconds.
...
\section{Future Work}
The most valuable next step would be an alignment step before grabbing, so that the parcel is always
taken at a known offset; this would address the largest source of delivery error directly. A
cancellation channel to the robot would close the gap described above. Collecting enough real runs
to retrain the classifier on measured rather than generated data would turn the machine-learning
component from a demonstration of a workflow into a genuine predictor, and requires no change to the
rest of the system because the file format is already shared. Finally, supporting more than one
robot would make the admission decision considerably more interesting, since it would then have to
choose which robot should take a job as well as whether the job is possible at all.

% ============================================================
\chapter{Conclusion}
% ============================================================

RoboFetch is a simulated warehouse delivery robot that decides whether it can perform a job before
accepting it. It satisfies the fifteen functional and six non-functional requirements set out in
Chapter~\ref{ch:requirements}, demonstrated by 65 automated tests and by fifteen acceptance checks
run against the complete live system, in which every physical claim was verified against the
simulator rather than against the software's own reports.

Measured against the categories the course requires, the system provides nine data-oriented
functionalities built on a normalised relational schema, integrates eight third-party services
ranging from a navigation stack to a machine-learning library, and contributes three complex
features of its own: a coupled model of the robot's physical condition, a multi-step admission
workflow that consults a learned model without being governed by it, and a verification mechanism
that cross-checks the software's claims against the physics engine.

Three lessons stand out from building it. The first is that a report of success is not evidence of
success; the incident in which the system confidently reported three deliveries that never happened
changed how the entire project was tested. The second is that testability is a consequence of
architecture rather than an activity performed at the end: the decision logic is fast to test
because it was written to depend on nothing. The third is that removing a feature can improve a
system; two elaborate components were deleted because the behaviour they produced did not justify
the complexity they carried, and the result is easier both to explain and to trust.
...
NFR1 & Timeout on the prediction client & Acceptance check NFR1, measured at 368 ms \\
...
The system starts from a single command, which stops anything left over from a previous run,
builds the workspace, and launches the simulator, the navigation stack, the robot nodes, the web
service and the prediction service.

Once the task manager reports that it is localized and ready, the interface is available in a
browser on the host machine. Orders can also be placed from a terminal using the provided script,
which shows the admission decision, follows the order through every state, and finishes by
comparing the parcel's real position with the delivery bay.

The automated tests run without the simulator. The acceptance suite requires a freshly launched
system and checks this itself before starting.
...
\ref{fig:balance} & Balance across the three functionality categories \\
...
\ref{fig:dbclass} & Database schema as a UML class diagram \\
\ref{fig:er} & Entity-relationship diagram \\
