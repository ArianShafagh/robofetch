...
{Component diagram in UML~2 notation. Every dependency is drawn as an \emph{assembly connector}:
a required interface (the socket) meeting a provided one (the ball), named for what actually
crosses that seam. The two components that face more than one direction carry explicit
\emph{ports} -- the Web Tier separates its browser, prediction and robot boundaries, and the Task
Manager separates the orders it receives from the actuation it commands. The Web Tier is the only component opened up, so that
\emph{delegation connectors} are visible once -- traffic arriving at a port is handed to an
internal part, and nothing outside addresses \texttt{db.py} or \texttt{admission.py} directly. The
others are drawn closed, which is the point of a component: what depends on it depends on its
interfaces, not on its insides. \texttt{robot\_model.py} is the exception that proves the rule --
it offers no interface at all and is reached by a \texttt{<<use>>} dependency, because it is a
library of equations shared by the robot and the web tier rather than a service either of them
calls.}{fig:component}
...
{Scenario 1: a controller places an order and it is delivered. The admission steps appear in the
order the code runs them: the robot's condition is read and projected forward past work already
accepted before any gate, and the classifier is gate 5, reached only because the four physical
checks passed. The task manager sends a destination, not a velocity: the navigation stack decides how to get there and drives the robot
itself, which is why it is the navigation stack and not the task manager that writes to the
velocity topic. The decision is taken before anything is stored, and the outcome is decided by reading the parcel's real position rather than by
...
{Scenario 2: the same request is refused because the robot could not return to its station
afterwards. The battery reads 80\%, which sounds ample until the return leg is costed: the heaviest
product over this route needs 69.6 percentage points of an 11 Wh pack, which would leave 10\% -- below
the 15\% that must remain. The gate order is the subject of this diagram: the reserve check is gate 2, it returns
immediately, and gates 3 to 5 never run -- so the \emph{classifier is not consulted at all}, and
its lifeline stays empty. That is the strongest form of the argument in Section~\ref{sec:c2}: the
model can refuse an order the physics allowed, but it is never in a position to approve one the
physics rejected. Nothing is written and nothing is published, so a refusal costs one preview and
there is nothing to undo.}{fig:seqrefused}

\uploadedfigure{Figures/12-sequence-emergency-stop.png}{0.95\textwidth}
{Scenario 3: the operator halts a running order. Inside the task manager the stop is handled on the
subscription callback rather than on the worker thread, because the worker is blocked waiting on
the navigation result at exactly the moment the button is pressed; the diagram shows the component
as one lifeline and leaves that division to the text. Two publishers write to the same velocity topic, which is why the halt is
republished rather than sent once: the navigation stack is already streaming velocity commands at
ten hertz, and a single zero would be overwritten by the next one before the cancellation had taken
effect. The web tier answers as soon as the stop has been published, before the robot has
physically halted, because an emergency control that blocks until the wheels have stopped is worse
than one that returns at once. Nothing is written by the
stop itself, so it does not survive a logout and the next order is an ordinary
one.}{fig:seqestop}

\uploadedfigure{Figures/13-sequence-model-refuses.png}{0.92\textwidth}
{Scenario 4: the four physical gates pass and the \emph{classifier} refuses. Read against
Figure~\ref{fig:seqrefused} this is the other half of the admission argument: there the physics
refused and the model was never reached, here the physics allows the job and only then is the model
asked. It refuses because 15.8\% remaining is barely above the 15\% reserve, and the model was
trained on jittered thresholds, so it treats the margin as too thin. Searching the whole input space
finds only four such orders, all with between 15.3\% and 16.8\% remaining -- the model disagrees
with the gates only in a sliver less than two points wide.}{fig:seqmodel}

\uploadedfigure{Figures/14-sequence-ai-unavailable.png}{0.92\textwidth}
{Scenario 5: the prediction service cannot be reached and the system decides anyway, which is NFR2
shown rather than asserted. The client times out at 1.5 s, inside the two-second budget NFR1 sets
for the whole preview, so a hanging model cannot break the response-time requirement. The four
physical gates are unchanged, and because the model was only ever able to refuse, losing it cannot
admit anything unsafe. The order records that it was decided without the model.}{fig:seqnoai}
...
\ref{fig:seqmodel} & Scenario 4: an order refused by the classifier, not by the gates \\
\ref{fig:seqnoai} & Scenario 5: a decision taken with the prediction service unreachable \\
