...
delivery and totals. \\
FR16 & A product that has been delivered shall be removed from stock and shall stop being
orderable. \\
FR17 & An accepted order shall reserve its projected energy and peak temperature until it reaches a
terminal state, and the next decision shall be taken against what would remain. \\
FR18 & A controller shall be able to queue a return to the station. It shall be costed and reserved
like any other task, and shall never be refused. \\
FR19 & A controller shall be able to halt the robot immediately, failing the running order and
everything queued. The halt shall not persist once it has been carried out. \\
FR20 & Every page shall require an authenticated session, and the two roles shall differ in what
they may reach, enforced on the request. \\
FR21 & An administrator shall be able to read any database table through the application, with
password hashes, salts and session tokens redacted. \\
\bottomrule
...
NFR1 & Preview (including the ML call) responds within 2 s & 343 ms \\
C2 & The classifier is consulted inside the admission workflow & ai\_service={'url': 'http://localhost:8001', 'reachable': True, 'error': None}, decided\_by=model \\
NFR3 & All pages render server-side with no JavaScript & 8 pages checked: \\
...
UC4 & An order can be tracked to a terminal state & order 1 -> completed: delivered (0.46 m from target) \\
C2 & The reservation is released when the order finishes & 0 order(s) still reserved after order 1 ended \\
UC5 & The parcel is physically delivered (Gazebo ground truth) & parcel\_1 at (-2.67, -1.88), 0.46 m from the bay, moved 3.24 m \\
...
FR10 & Battery, temperature and condition are tracked & battery 74.1\%, temp 44.9 C, condition 99.97\%, state=idle \\
FR12 & The run is logged to CSV for ML training & run\_20260818\_223755.csv: 41 samples \\
FR5 & An order the robot cannot serve is refused, with a reason & status=refused, reason='SKU-1002 is out of stock' \\
FR11 & An idle robot returns to its station and recharges & idle at 74.1\% -> docked and charging, 69.3\% -> 85.0\% \\
FR6 & Orders are served oldest-first (FIFO) & submitted [4, 5], started [4, 5] - the earlier order ran first \\
FR8 & A failing grab is retried 3 times, then the order fails & order 6 -> failed after 3 attempts: could not grab parcel\_6 in 3 attempts; returning to station \\
...
UC11 & The interrupted order is failed, not left pending & order 7 -> failed: halted by the emergency stop \\
UC11 & Nothing stays engaged - the robot accepts work immediately after & next order -> accepted: '8.3 m, 3.4 Wh, ~54 s; battery 43\% after returning to the station' \\
UC6 & Analytics agree with the orders table & reported 3/8 completed and 2 refused; counted 3 and 2; rate=0.5 \\
\bottomrule
...
condition model forward and calling the prediction service over HTTP, was measured at 343
...
redaction \\
NFR1 & Timeout on the prediction client & Acceptance check NFR1, measured at 343 ms \\
